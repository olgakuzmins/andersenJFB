...
public class TicketDAO {
    ...
    public void saveTicket(Ticket ticket, User user) {
        try (Connection connection = ConnectionConfig.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(INSERT_TICKET_WITH_USER_QUERY)) {
    ...
    public Ticket fetchTicketById(UUID id) {
        ...
        try (Connection connection = ConnectionConfig.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(SELECT_TICKET_BY_ID_QUERY)) {
    ...
    public List<Ticket> fetchTicketsByUserId(UUID userId) {
        ...
        try (Connection connection = ConnectionConfig.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(SELECT_TICKETS_BY_USER_ID_QUERY)) {
    ...
    public Ticket fetchTicketByIdAndUserId(UUID id, UUID userId) {
        Ticket ticket = null;
        try (Connection connection = ConnectionConfig.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(SELECT_TICKETS_BY_ID_AND_USER_ID_QUERY)) {
            preparedStatement.setObject(1, id);
            ...
            if (resultSet.next()) {
                ...
                ticket.setUserId(UUID.fromString(resultSet.getString("user_id")));
                ticket.setType((TicketType.valueOf(resultSet.getString("ticket_type"))));
                ticket.setTime(resultSet.getTimestamp("creation_date").toInstant());
            }
        } catch (SQLException e) {
    ...
    public void updateTicketType(UUID id, TicketType type) {
        try (Connection connection = ConnectionConfig.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(UPDATE_TICKET_TYPE_BY_ID_QUERY)) {
